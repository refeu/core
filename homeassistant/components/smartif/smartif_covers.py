...
class SmartIfCovers:
    ...
    async def open_cover(self, _id: str) -> None:
        """Open the cover."""
        await self.smart_if.request(f"Covers/{_id}/OpenCover", method=METH_POST)

    async def close_cover(self, _id: str) -> None:
        """Close cover."""
        await self.smart_if.request(f"Covers/{_id}/CloseCover", method=METH_POST)

    async def set_cover_position(self, _id: str, position: int) -> None:
        """Move the cover to a specific position."""
        await self.smart_if.request(
            f"Covers/{_id}/SetCoverPosition?position={position}", method=METH_POST
        )

    async def stop_cover(self, _id: str) -> None:
        """Stop the cover."""
        await self.smart_if.request(f"Covers/{_id}/StopCover", method=METH_POST)
